export const objectcssnavbarmobil = [
  {
    id: 1,
    title: "Home",
    generalcss:
      "bg-white flex justify-center items-center col-start-[3] col-end-[27] row-start-2 row-end-5 text-black rounded-lg gap-3",
    screenxl: "",
    screenlg: "",
    screensm: "",
    screenmd: "",
    modalopencss: "bg-red-600",
    dataimg: {
      src: "../src/assets/home-real-estate-svgrepo-com.svg",
      alt: "home",
      width: "40",
      height: "40",
    },
    rutas: [
      {
        id: 1,
        ruta: "",
        title: "lofy",
        css: "bg-neutral-950 flex justify-center items-center text-black col-start-5 col-end-[25] row-start-2 row-end-4 rounded-[3px] text-white",
      },
      {
        id: 2,
        ruta: "",
        title: "dark",
        css: "bg-neutral-950 flex justify-center items-center text-black col-start-5 col-end-[25] row-start-4 row-end-6 rounded-[3px] text-white",
      },
    ],
  },
  {
    id: 2,
    title: "Temario",
    generalcss:
      "bg-white flex justify-center items-center col-start-[3] col-end-[27] row-start-5 row-end-[8] text-black rounded-lg",
    screenxl: "",
    screenlg: "",
    screensm: "",
    screenmd: "",
    modalopencss: "bg-red-600",
    dataimg: {
      src: "../src/assets/index-svgrepo-com.svg",
      alt: "temario",
      width: "40",
      height: "40",
    },
    rutas: [
      {
        id: 1,
        ruta: "",
        title: "party",
        css: "bg-neutral-950 flex justify-center items-center text-black col-start-5 col-end-[25] row-start-2 row-end-4 rounded-[3px] text-white",
      },
    ],
  },
  {
    id: 3,
    title: "Guia",
    generalcss:
      "bg-white flex justify-center items-center col-start-[3] col-end-[27] row-start-[8] row-end-[11] text-black rounded-lg gap-4",
    screenxl: "",
    screenlg: "",
    screensm: "",
    screenmd: "",
    modalopencss: "bg-red-600",
    dataimg: {
      src: "../src/assets/document-svgrepo-com.svg",
      alt: "guia",
      width: "40",
      height: "40",
    },
    rutas: [
      {
        id: 1,
        ruta: "",
        title: "party",
        css: "bg-neutral-950 flex justify-center items-center text-black col-start-5 col-end-[25] row-start-2 row-end-4 rounded-[3px] text-white",
      },
    ],
  },
  {
    id: 4,
    title: "Glosario",
    generalcss:
      "bg-white flex justify-center items-center col-start-[3] col-end-[27] row-start-[11] row-end-[14] text-black rounded-lg",
    screenxl: "",
    screenlg: "",
    screensm: "",
    screenmd: "",
    modalopencss: "bg-red-600",
    dataimg: {
      src: "../src/assets/search-magnifying-glass-svgrepo-com.svg",
      alt: "search",
      width: "40",
      height: "40",
    },
    rutas: [
      {
        id: 1,
        ruta: "",
        title: "party",
        css: "bg-neutral-950 flex justify-center items-center text-black col-start-5 col-end-[25] row-start-2 row-end-4 rounded-[3px] text-white",
      },
    ],
  },
  {
    id: 5,
    title: "UI Tema",
    generalcss:
      "bg-white flex justify-center items-center col-start-[3] col-end-[27] row-start-[14] row-end-[17] text-black rounded-lg",
    screenxl: "",
    screenlg: "",
    screensm: "",
    screenmd: "",
    modalopencss: "bg-red-600",
    dataimg: {
      src: "../src/assets/gradient-design-svgrepo-com.svg",
      alt: "gradient",
      width: "40",
      height: "40",
    },
    rutas: [
      {
        id: 1,
        ruta: "",
        title: "party",
        css: "bg-neutral-950 flex justify-center items-center text-black col-start-5 col-end-[25] row-start-2 row-end-4 rounded-[3px] text-white",
      },
    ],
  },
];
